...
class GAN:
    ...
    def build_model(self):
        ...
        self.t_real_image = tf.placeholder('float32', [self.options['batch_size'], img_size, img_size, 3], name='real_image')
        self.t_wrong_image = tf.placeholder('float32', [self.options['batch_size'], img_size, img_size, 3], name='wrong_image')
        self.t_real_caption = tf.placeholder('float32', [self.options['batch_size'], self.options['caption_vector_length']], name='real_caption_input')
        self.t_z = tf.placeholder('float32', [self.options['batch_size'], self.options['z_dim']])

        self.fake_image = self.generator(self.t_z, self.t_real_caption)

        self.disc_real_image, self.disc_real_image_logits = self.discriminator(self.t_real_image, self.t_real_caption)
        self.disc_wrong_image, self.disc_wrong_image_logits = self.discriminator(self.t_wrong_image, self.t_real_caption, reuse=True)
        self.disc_fake_image, self.disc_fake_image_logits = self.discriminator(self.fake_image, self.t_real_caption, reuse=True)

        self.g_loss = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.disc_fake_image_logits, logits=tf.ones_like(self.disc_fake_image)))

        self.d_loss1 = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.disc_real_image_logits, logits=tf.ones_like(self.disc_real_image)))
        self.d_loss2 = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.disc_wrong_image_logits, logits=tf.zeros_like(self.disc_wrong_image)))
        self.d_loss3 = tf.reduce_mean(
            tf.nn.sigmoid_cross_entropy_with_logits(labels=self.disc_fake_image_logits, logits=tf.zeros_like(self.disc_fake_image)))

        self.d_loss = self.d_loss1 + self.d_loss2 + self.d_loss3
        ...
        outputs = {
            'generator': self.fake_image
        }

        checks = {
            'd_loss1': self.d_loss1,
            'd_loss2': self.d_loss2,
            'd_loss3': self.d_loss3,
            'disc_real_image_logits': self.disc_real_image_logits,
            'disc_wrong_image_logits': self.disc_wrong_image,
            'disc_fake_image_logits': self.disc_fake_image_logits
        }
